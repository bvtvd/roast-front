<template>
    <div id="cafe-filter">
        <div class="grid-container" v-show="show">
            <div class="grid-x grid-padding-x">
                ...
                <div class="brew-methods-container">
                    <div class="filter-brew-method" @click="toggleBrewMethodFilter(method.method)" :class="{'active': brewMethods.indexOf(method.method) > -1}" v-for="method in cafeBrewMethods" :key="method.id">
                        {{ method.method }}
                    </div>
                </div>
            </div>
        ...
        <div class="grid-container">
            <div class="grid-x grid-padding-x">
                <span class="show-filters" @click="show = !show">{{ show ? '隐藏过滤器↑' : '显示过滤器↓'}}</span>
            </div>
        </div>
    </div>
</template>
